...
| `week` | 週キー (YYYY-Www) | この週にやる |
| `none` | NULL | いつかやる (Future) |

週キーの定義: 週境界は設定の「週の開始曜日」に従うため、純粋な ISO 週とはずれうる。
そこで**「その週の 4 日目が属する ISO 週」をその週のラベルとする**(月曜始まり(既定)では
ISO 8601 と完全一致し、どちらの設定でも `(year, week)` の順序は日付順と一致する)。
注意: 週開始曜日を後から変更すると既存の `week(w)` 値の解釈が最大 1 週ずれるが、
表示バケットが 1 段ずれるだけで実害が小さいため、設定変更時のマイグレーションは行わない。
