...
1. **メインウィンドウ**: Trello 風ボード。閉じるボタンはトレイへの格納(終了はトレイメニューから)。
   ...
   - Tomorrow / This Week / Next Week / Future / Icebox / Watching は「すぐに気にしなくてよいことの
     置き場」なので、通常表示では列として展開せず、**ドロップ先の箱 + 件数インジケーター**として
     小さく表示する(通常表示の列は New / Today / Doing / Done)。全列を展開する表示モードも用意する。
     展開表示では Icebox を**一番左**に置く(トリアージ前の在庫という位置づけのため)。
     Icebox は「いつやるかの判断ごと後回し」の置き場で、Watching と違い外部の変化でも自動では動かない。
   - Watching は外部の変化待ち(data-model.md 参照)。外部 origin の変化で自動的に New へ起床し、
     オーバーレイ通知が出る。GitHub プラグインは対象タスクが Watching のとき、通知子タスクを
     作る代わりに履歴追記で本体を起床させる。
...
- シークレット(MCP トークン・プラグインの `type: "secret"` 設定)は Windows 資格情報マネージャー
  (keyring crate、service 名 "questloom")に保存する。DB には置かない。
  旧バージョンが settings テーブルに残した平文値は起動時に一度だけ移送される。
  平文フォールバックはせず、資格情報ストアが使えない場合は保存エラーとして扱う。
  詳細と既知の制限は CLAUDE.md の「シークレットの保存先」節を参照。
